...
void WrapOpen(const char *argIn, char *argOut) {
    ...
    int rc = 0;
    if(-1 == out_flags){
        rc = unixOpen(&aVfs[0], path, (sqlite3_file *) &file_infor, in_flags, NULL);
    }else {
        rc = unixOpen(&aVfs[0], path, (sqlite3_file *) &file_infor, in_flags, &out_flags);
    }
    printf("   id.h = %d\n", file_infor.h);
    unixOpenConvertReturnToChar((sqlite3_file *) &file_infor, &out_flags, &rc, argOut);

    printf("pMethods = %d\n", ((sqlite3_file *) &file_infor)->pMethods);
...
void WrapNolockClose(const char *argIn, char *argOut) {
    printf("---WrapUnfetch:\n");
    unixFile id;
    i64 iOff;
    int p_flag;
