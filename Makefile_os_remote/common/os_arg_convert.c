...
struct ArgInNolockClose {
    char file_infor[SIZE_UNIXFILE];
};

typedef struct ArgInNolockClose ArgInNolockClose;

struct ReturnNolockClose {
    char file_infor[SIZE_UNIXFILE];
    int rc;
};
typedef struct ReturnNolockClose ReturnNolockClose;

void nolockCloseConvertArgInToChar(sqlite3_file *fd, char *arg) {
    memcpy(arg, fd, SIZE_UNIXFILE);
}

void nolockCloseConvertCharToReturn(const char *arg, sqlite3_file *fd, int *pRc) {
    memcpy(fd, arg, SIZE_UNIXFILE);
    memcpy(pRc, arg + SIZE_UNIXFILE, sizeof(int));
}

void nolockCloseConvertCharToArgIn(char *arg, sqlite3_file *fd) {
    memcpy(fd, arg, SIZE_UNIXFILE);
}

void nolockCloseConvertReturnToChar(sqlite3_file *fd, int *pRc, char *arg) {
    memcpy(arg, fd, SIZE_UNIXFILE);
    memcpy(arg + SIZE_UNIXFILE, pRc, sizeof(int));
}
